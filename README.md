...
`risk_engine/remediation_engine.py` triggers bash-based playbooks defined in `risk_engine/config/remediations.yaml` when certain threat types match. Scripts are executed inside the affected container via Docker `exec_run` with `user="root"`. Results are logged as `AUTO_REMEDIATION` events visible in the timeline.

| Threat Type | Playbook | Action |
|---|---|---|
| `RUNTIME_DRIFT` | Kill Unauthorized Process | `ps aux` → `kill -9 <PID>` for rogue processes |
| `UNEXPECTED_NETWORK_ATTACH` | Isolate Rogue Network Segment | `iptables -A DROP` for rogue subnet |
| `CONFIG_DRIFT` | Restore Configuration Baseline | `cp /etc/config.bak /etc/config` |
| `LATERAL_MOVEMENT` | Revoke SSH Keys & Reset | `pkill -9 sshd` + credential rotation |
| `IMAGE_MISMATCH` | Verify Container Digest & Restart | Re-pull approved image + restart service |
