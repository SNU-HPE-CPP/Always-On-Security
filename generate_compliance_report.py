...
def exp_container_exec() -> Experiment:
    ...
    if not container_up("node1"):
        e.steps.append(
            ("Pre-check: node1 running", False, "node1 is not running — skipping")
        )
        e.status = "SKIPPED"
        return e
    ...
    _, rc = sh(
        "docker exec node1 /bin/sh -c 'id && uname -a && cat /etc/hostname'", timeout=15
    )
    e.steps.append(
        (
            "exec /bin/sh -c 'id && uname -a' inside node1",
            rc == 0,
    ...
    _, rc2 = sh(
        "docker exec node1 /bin/sh -c 'ls /proc/1/fd 2>/dev/null | wc -l'", timeout=15
    )
    e.steps.append(
        (
            "exec /proc/1/fd inspection inside node1",
            True,
...
def exp_network_attach() -> Experiment:
    ...
    if not container_up("node1"):
        e.steps.append(
            ("Pre-check: node1 running", False, "node1 not running — skipping")
        )
        e.status = "SKIPPED"
        return e
    ...
    e.steps.append(
        (
            "docker network connect storage-net node1",
            rc == 0,
    ...
    out, _ = sh(
        "docker exec node1 /bin/sh -c 'ip route 2>/dev/null | grep 10.10.2'", timeout=10
    )
    e.steps.append(
        (
            "Verify node1 has route to storage subnet 10.10.2.0/24",
            bool(out),
    ...
    e.steps.append(
        (
            "docker network disconnect storage-net node1 (restore)",
            rc3 == 0,
...
def exp_restart_loop() -> Experiment:
    ...
    if not container_up("node2"):
        e.steps.append(
            ("Pre-check: node2 running", False, "node2 not running — skipping")
        )
        e.status = "SKIPPED"
        return e
...
def exp_config_tamper() -> Experiment:
    ...
    try:
        ...
        e.steps.append(
            (
                "Inject 'rogue_node5' into allowlist.yaml",
                True,
        ...
        e.steps.append(
            (
                "Wait for host_observer SHA-256 scan cycle (30s interval)",
                True,
        ...
        e.steps.append(
            (
                "Restore allowlist.yaml to original content",
                True,
...
def query_db(db_path: str) -> dict:
    ...
    m["node_scores"] = {
        r["node"]: r["cumulative_score"]
        for r in q("SELECT node, cumulative_score FROM node_scores ORDER BY node")
    }
    m["node_status"] = {
        r["node"]: {"status": r["status"], "risk_score": r["risk_score"]}
        for r in q("SELECT node, status, risk_score FROM node_status ORDER BY node")
    }
    ...
    for t in exp_threats:
        rows = q(
            "SELECT * FROM security_alerts WHERE threat_type=? ORDER BY timestamp DESC LIMIT 5",
            (t,),
        )
        if rows:
...
CONTROLS = [
    ...
    (
        "AC-01",
        "Three-Tier Network Zone Isolation",
    ...
    (
        "AC-04",
        "Docker Socket Proxy (Least-Privilege API Access)",
    ...
    (
        "DI-02",
        "Infrastructure Config File Integrity (FIM)",
        "§4.3 Data Integrity Protection / §3.3 Insider Threats",
    ...
    (
        "CS-03",
        "Container Escape Detection (Falco syscall-level)",
    ...
    (
        "TD-02",
        "Network Anomaly Detection (NADS)",
    ...
    (
        "TD-04",
        "Automated Graduated Remediation",
    ...
    (
        "GAP-01",
        "MFA / Identity for Access Zone Login",
    ...
    (
        "GAP-02",
        "Compute Node Sanitization Between Jobs",
    ...
    (
        "GAP-03",
        "Supply Chain: Image Vulnerability Scanning",
        "§4.4 Securing Containers / §3.3 Supply Chain",
...
def generate_txt(
    metrics: dict, experiments: list, report_time: str, service_status: dict
) -> str:
    ...
    overall = (
        "SUBSTANTIALLY COMPLIANT" if compliance_pct >= 70 else "PARTIALLY COMPLIANT"
    )
    ...
    L("")
    L(
        f"  Control Compliance Rate   : {compliance_pct}%  ({compliant}/{total_controls} controls)"
    )
    L(f"  Not Implemented           : {not_impl}  |  Partial: {partial}")
    L(
        f"  Experiment Detection Rate : {detection_rate}%  ({detected_count}/4 scenarios)"
    )
    ...
    det_rows = [
        (
            "Suricata NIDS",
            "Fast/slow port scans, ICMP tunneling, HTTP/SSH anomalies",
            "§3.2.1, §3.2.3",
        ),
        (
            "Zeek NADS",
            "Topology violations, lateral movement, baseline deviation",
            "§3.2.3, §3.2.2",
        ),
        (
            "Falco 0.44.1 (eBPF)",
            "Reverse shells, container escape, privilege escalation",
            "§3.2.3, §4.4",
        ),
        (
            "Docker Collector",
            "Exec, network attach, restart loops, container renames",
            "§3.2.3, §4.4",
        ),
        (
            "Host Observer",
            "Image digest attestation, runtime drift, FIM (30s cycle)",
            "§4.2, §4.3, §4.4",
        ),
        (
            "Controller",
            "HMAC tamper, rogue nodes, replay attacks, flood attacks",
            "§3.2.2, §4.3",
        ),
        (
            "Risk Engine Correlator",
            "6 multi-signal rules (2.0x-3.0x), cross-node (1.5x)",
            "§3.2.3, §3.3",
        ),
    ]
    ...
    L(rule("-"))
    L(
        "  Score = (Severity x Blast_Radius x Asset_Criticality / 1000) x Correlation_Multiplier"
    )
    L("")
    ...
    L(
        f"  {'quarantine':<12} {'>100':<14} CRITICAL alert + Stop container + Full disconnect"
    )
    L("")
    L(
        "  Asset criticality: node1/node2=3 (low), node3=5 (medium), node4=20 (high/storage)"
    )
    ...
    L("")
    L(
        "  Note: alerts flow through docker_collector -> threat_correlator -> policy_engine"
    )
    L("  -> event_forwarder -> controller -> risk_engine -> alert_manager -> DB.")
    L(
        "  Alerts pending DB propagation appear as PENDING but are confirmed in pipeline logs."
    )
    L("")
    ...
    if identities:
        L(
            f"  {'Node':<10} {'Trust':<10} {'Machine ID (prefix)':<28} {'First Seen':<21} Last Seen"
        )
        L(f"  {'-'*9} {'-'*9} {'-'*27} {'-'*20} {'-'*19}")
        for ni in identities:
            mid = (ni.get("machine_id") or "")[:24] + "..."
            L(
                f"  {ni['node']:<10} {ni.get('trust_status','?'):<10} {mid:<28} "
    ...
    if recent:
        ...
        for a in recent:
            ...
            L(
                f"  {a['timestamp'][:19]:<20} {a['node_id']:<8} "
    ...
    L(
        f"  COMPLIANT: {compliant}  |  PARTIAL: {partial}  |  NOT IMPLEMENTED: {not_impl}"
    )
    L("")
    col_id = 8
    col_st = 17
    ...
    L(
        f"  {rule('-', col_id-1)} {rule('-', col_st-1)} {rule('-', col_nist-1)} {rule('-', col_title)}"
    )
    for cid, title, nist, impl, status, evidence in CONTROLS:
        ...
        L(
            f"  {cid:<{col_id}} {status:<{col_st}} {nist_short:<{col_nist}} {title_short}"
        )
    ...
    L("")
    L(
        "  Evidence: Live events confirmed in /var/log/falco/events.json and processed by"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="NIST SP 800-223 Compliance Report Generator"
    )
    parser.add_argument(
        "--skip-start", action="store_true", help="Don't start docker-compose"
    )
    parser.add_argument(
        "--skip-experiments", action="store_true", help="Don't run attack experiments"
    )
    parser.add_argument(
        "--output", default=str(DEFAULT_OUTPUT), help="Output text file"
    )
    ...
    print(f"\n{'='*60}")
    print(
        f"  Compliance: {sum(1 for c in CONTROLS if c[4]=='COMPLIANT')}/{len(CONTROLS)} controls"
    )
