...
class Store:
    ...
    def _init_schema(self):
        with self.lock:
            ...
            try:
                c.execute(
                    "ALTER TABLE node_status ADD COLUMN isolated_ip TEXT DEFAULT NULL"
                )
            except sqlite3.OperationalError:
                pass
        ...
        with self.lock:
            ...
            """,
                (
                    ...
                    (
                        fim.get("current_state", {}).get("sha256")
                        if fim.get("current_state")
                        else None
                    ),
                    (
                        fim.get("current_state", {}).get("file_size")
                        if fim.get("current_state")
                        else None
                    ),
                    (
                        fim.get("current_state", {}).get("permissions")
                        if fim.get("current_state")
                        else None
                    ),
                ),
    ...
    def update_node_status(
        self, node: str, status: str, risk_score: float, isolated_ip: str = None
    ) -> None:
        with self.lock:
        ...
        with self.lock:
            ...
            """,
                (
                    ...
                    node,
                    "NODE_UNRESPONSIVE",
                    json.dumps([f"Node silent for {delta_seconds:.0f}s"]),
                    100.0,
                ),
            )
            c.execute(
    ...
    def get_replay_count(self, node: str = None) -> int:
        if node:
            ...
            row = self.conn.execute("SELECT COUNT(*) as c FROM replay_log").fetchone()
